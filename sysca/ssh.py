...
_WORKING_MVIEW = hasattr(x509, "IssuingDistributionPoint")
...
def _init_cipher(ciphername, password, salt, rounds, backend):
    ...
    # bcrypt does not support buffer protocol
...
class _SSHFormatECDSA:
    ...
    def encode_public(self, public_key, f_pub):
        ...
        if hasattr(Encoding, "X962"):
            point = public_key.public_bytes(
                Encoding.X962, PublicFormat.UncompressedPoint
            )
        elif hasattr(public_key, "encode_point"):
            point = public_key.encode_point()
        else:
...
def _lookup_kformat(key_type):
    ...
    """
    if isinstance(key_type, memoryview):
        key_type = key_type.tobytes()
...
def load_ssh_private_key(data, password, backend):
    ...
    """
    p1 = data.find(_SK_START)
    if p1 >= 0:
        p2 = data.find(_SK_END, p1 + len(_SK_START))
        if p2 > p1:
            data = memoryview(data)
            data = binascii.a2b_base64(data[p1 + len(_SK_START):p2])
    ...
    if (ciphername, kdfname) != (_NONE, _NONE):
        ...
        if _WORKING_MVIEW:
            ...
            edata = memoryview(ciph.decryptor().update(edata.tobytes()))
    else:
        blklen = 8
        _check_block_size(edata, blklen)
...
def serialize_ssh_private_key(private_key, password=None):
    ...
    if isinstance(private_key, ec.EllipticCurvePrivateKey):
        ...
        key_type = _SSH_DSA
    elif ed25519 is not None and isinstance(private_key, ed25519.Ed25519PrivateKey):
    ...
    if ciph is not None:
        if _WORKING_MVIEW:
            ...
            buf[ofs:mlen] = ciph.encryptor().update(buf[ofs:mlen].tobytes())
